...
func TestRenderInDeployService(t *testing.T) {
	...
	testDataReturn := func(cluster, service string) (*types.ServiceStatus, error) {
	...
	m := New("test-cluster", "test-service", "service-arn", nil)

	m.ecsStatus, _ = testDataReturn("", "")
	m.state = loaded
